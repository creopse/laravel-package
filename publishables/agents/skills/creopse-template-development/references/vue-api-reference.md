...
| `getContentModelItems` | `(name: string, activeOnly?: boolean, sortBy?: string, sortDirection?: SortDirection) => Promise<ContentModelItemModel[]>` | Items of a content model (`activeOnly` default `true`). Pass `sortBy: 'position'` to respect the admin's drag-and-drop order. |
