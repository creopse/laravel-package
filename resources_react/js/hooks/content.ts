...
export const useContent = () => {
   ...
   * @returns {object | any | null} - The data of the section, or `null`.
   */
  const getSectionSettings = (key: string | null | undefined): object | any | null => {
    if (!key) return null

    const keyParts = key.split('__')
    const slug = keyParts.length ? keyParts[0] : ''
    const linkId = keyParts.length > 1 ? keyParts[1] : ''
    return page.props.pageData?.sections?.find((section) => section.slug == slug && section.pivot?.linkId == linkId)?.pivot?.settings || null
  }
   ...
   * @returns {object | any | null} - The data of the section, or `null`.
   */
  const getSectionSettingsGroup = (key: string | null | undefined, group: string): object | any | null => {
   ...
   */
  const getSectionSetting = (key: string | null | undefined, group: string, name: string): object | any | null => {
   ...
   */
  const getAnySectionData = async (sectionSlug: string, pageSlug: string, linkId: string = 'default') => {
